...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (self.viewMode == ViewModeLocal) {
        ...
        [xiangXiXinXiCell configureOnKey:keyValues withField:rowValues andContent:[self.myRecipe valueForKey:keyValues]];
	} else {
		[xiangXiXinXiCell configureOnKey:keyValues withField:rowValues andContent:[self.myRecipeOnServer valueForKey:keyValues]];
	}
...
- (IBAction)deleteCurrentDraft:(id)sender {
    UIAlertView *deleteAlertView = [[UIAlertView alloc] initWithTitle:@"警告" message:@"您真的要Delete该条信息吗？\n该操作无法恢复！" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
...
- (IBAction)enterPhotoWall:(id)sender {
	if (self.isNeedUpdateImages) {
		...
		[self initImageData:self.myRecipeOnServer.objectId WithCompletionBlock:^{
			[SVProgressHUD dismiss];
...
{
	AVObject *infoToBeUpdate = [AVObject objectWithoutDataWithClassName:DPServerTableMyRecipe objectId:self.myRecipeOnServer.objectId];
...
- (void)tianJiaController:(TianJiaController *)tianJiaController didEditObject:(id)object
{
	if (self.viewMode == ViewModeLocal) {
		...
		self.myRecipeOnServer = (AVObject *)object;
	}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([[segue identifier] isEqualToString:@"EditLocalInfo"]) {
        ...
        tianJiaController.myRecipe = self.myRecipe;
