...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.isEditMode) {
        ...
        if (self.isLocal) {
            ...
            [tianJiaCell configureOnKey:keyValues withField:rowValues andContent:[self.myRecipe valueForKey:keyValues]];
        } else {
...
//    ShangHu *shangHu = nil;
    ...
    [self setFieldForObject:myRecipe];
    ...
    [localContext MR_saveToPersistentStoreWithCompletion:^(BOOL success, NSError *error) {
        if (error) {
            ...
            [self.delegate tianJiaController:self didEditObject:myRecipe];
            [self.navigationController popViewControllerAnimated:YES];
        }
    }];
}
...
- (void)updateSeverInfo {
    AVObject *shangHuOnServerToBeUpdate = [AVObject objectWithoutDataWithClassName:DPServerTableMyRecipe objectId:self.shangHuOnServer.objectId];
